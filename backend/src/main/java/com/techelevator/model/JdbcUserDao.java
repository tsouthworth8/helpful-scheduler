...
public class JdbcUserDao implements UserDao {
	 ...
	 */
	@Override
	public User saveUser(String userName, String email, String password, boolean isManager, long companyId) {
		byte[] salt = passwordHasher.generateRandomSalt();
		String hashedPassword = passwordHasher.computeHash(password, salt);
		String saltString = new String(Base64.encode(salt));
		long newId = jdbcTemplate.queryForObject(
				"INSERT INTO users(username, email, password, salt, manager, company_id) VALUES (?, ?, ?, ?, ?, ?) RETURNING id",
				Long.class, userName, email, hashedPassword, saltString, isManager, companyId);
	...
	public long getCompanyIdByUserId(long userId) {
		String sqlCompanyId = "SELECT company_id FROM users WHERE id = ?";
		SqlRowSet results = jdbcTemplate.queryForRowSet(sqlCompanyId, userId);
